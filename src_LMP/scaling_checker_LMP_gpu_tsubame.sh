...
# LMPの node内の並列は一般的に高くないので、ここでは、OPENMPは使わない
jobscript=jobscript_LMP_template
LMP_input_array=(in_128000atm in_256000atm in_32000atm in_64000atm)
process_per_node=(1 2 4 8 16 24 40 48 80 96 160 192 )  
...
# CCCCCC -> ${LMP_input_array[@]} : LMP_input
...
for type_node in "${node_type[@]}"; do
    for num_node in "${number_nodes[@]}"; do
        ...
        for LMP_input in "${LMP_input_array[@]}" 
        do
            ...
            do    
                ...
                if [[ $num_cores -gt $((processes * num_node)) ]]; then
                    sed -e "s/AAAAAA/${type_node}=${num_node}/g" \
                        -e "s/BBBBBB/np${processes}cpu${num_cores}gpu${num_gpu}_${LMP_input}/g" \
                        -e "s/CCCCCC/${LMP_input}/g" \
                        -e "s/DDDDDD/${processes}/g" \
                        -e "s/EEEEEE/$((processes * num_node))/g" \
                        -e "s/FFFFFF/${num_gpu}/g" \
                        "$jobscript".sh > "LMPjob_${processes}np_${num_cores}cpu_${num_gpu}gpu_${LMP_input}.sh"
                fi
            done
        done
